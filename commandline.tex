...
\item Tar just concatinates files into one blob, so other helper programs such as gzip and bzip2 are used to compress the blob.
\pause
...
\item aptitude handles all the package fetching and dependancy resolution for you, in a safe and easy way.
...
\item \texttt{\color{blue}brew install \$(brew oudated)} upgrades outdated pacakges.
\end{itemize}
\end{frame}
...
\item The two big command line editors are \texttt{vim} and \texttt{emacs}, which are very different and have whole communities behind them.
\item Another smaller editor is \texttt{nano}, which is less powerful, but more accessible.
...
\item The two major players are \texttt{git} and \texttt{svn}.
\item The commands for both are near identical, and you'll have \texttt{svn} experience in classes, so we will only cover \texttt{git} here.
\end{itemize}
\end{frame}
...
\item To read a man page, just type \texttt{man} followed by the program name to launch it's manual.
...
\item \texttt{man sh} should give you all the syntax.
